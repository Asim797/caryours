...
<div class="d-flex align-items-center mt-lg-4 mt-3">
    <button class="action-btn-sm me-2" @click="adsDetailsDownload(data.id)"><i><svg id="print" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 43.344 43.346">
        <path id="Path_145" data-name="Path 145" d="M37.606,11.248V1.125H7.988V11.248a7.233,7.233,0,0,0-6.863,7.215V35.8h6.14v-2.89H4.015V18.463a4.34,4.34,0,0,1,4.334-4.334h28.9a4.34,4.34,0,0,1,4.334,4.334V32.911H37.606V35.8h6.863V18.463A7.233,7.233,0,0,0,37.606,11.248Zm-2.89-.009H10.878V4.015h23.84Z" transform="translate(-1.125 -1.125)" fill="#0586f4" />
        <path id="Path_146" data-name="Path 146" d="M27.844,14.063h2.89v2.89h-2.89Z" transform="translate(6.471 2.553)" fill="#0586f4" />
        <path id="Path_147" data-name="Path 147" d="M8.955,18.563H5.344v2.89H8.957V39.513H33.519V21.453h3.613v-2.89ZM30.628,36.624H11.845V21.453H30.628Z" transform="translate(0.074 3.832)" fill="#0586f4" />
    </svg>
</i></button>
<a target="_blank" :href="`https://www.facebook.com/sharer/sharer.php?u=${base_url}/site-manager/web/category/ads/${this.$route.params.id}`" class="action-btn-sm me-2">
    <i class="fas fa-share p-md"></i>
</a>

<!-- <button class="action-btn-sm me-2"><i class="fas fa-share p-md"></i></button> -->
<button
v-if="isLoggedIn() && data.is_wishlisted === false"
class="action-btn-sm me-2"
@click="addWhislist(data.id)"
>
<i class="far fa-heart blue-text" ></i>
</button>
<button v-else="isLoggedIn() && data.is_wishlisted === true"
class="action-btn-sm me-2"
@click="removeWhislist(data.id)"
>
...
export default {
    ...
    methods: {
        ...
        async adsDetailsDownload(recipeId){
	            let urlPath = `${window.base_url}/api/advertisements/ads-details/${recipeId}`;
	            //alert(urlPath);
